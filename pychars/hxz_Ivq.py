...
def get_res_var(df, firm_list):
    ...
    for firm, count, prog in zip(firm_list['permno'], month_num, range(firm_list['permno'].count()+1)):
        ...
        for i in range(count + 1):
            ...
            # if observations in last 1 month are less 15, we drop the rvar of this month
            if temp['permno'].count() < 15:
                ...
                X = pd.DataFrame()
                temp['rmf'] = temp['R_MKT'] - temp['R_F']
                temp['y'] = temp['ret'] - temp['R_F']
                X[['rme']] = temp[['R_ME']]
                X[['rm_f']] = temp[['rmf']]
                X[['ria']] = temp[['R_IA']]
                X[['roe']] = temp[['R_ROE']]
                X['intercept'] = 1
                X = X[['intercept', 'rm_f','rme','ria','roe']]
                X = np.mat(X)
                Y = np.mat(temp[['y']])
...
crsp = crsp.rename(columns={'rvar': 'Ivq'})
crsp = crsp.reset_index(drop=True)
crsp = crsp[['permno', 'date', 'Ivq']]

with open('Ivq.pkl', 'wb') as f:
    pkl.dump(crsp, f)